...
class OwnerRestControllerTests {
    ...
    @Test
    @WithMockUser(roles = "OWNER_ADMIN")
    void testCreatePetWithDecimalWeight() throws Exception {
        PetDto newPet = pets.get(0);
        newPet.setId(777);
        newPet.setWeight(15.75); // Test decimal precision
        ObjectMapper mapper = new ObjectMapper();
        mapper.registerModule(new JavaTimeModule());
        mapper.setDateFormat(new SimpleDateFormat("yyyy-MM-dd"));
        mapper.disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS);
    ...
    void testUpdateOwnersPetWithWeight() throws Exception {
        ...
        updatedPetDto.setName("Rex");
        updatedPetDto.setBirthDate(LocalDate.of(2020, 1, 15));
        updatedPetDto.setWeight(32.75); // Update weight
    ...
    void testUpdateOwnersPetWithNullWeight() throws Exception {
        ...
        updatedPetDto.setName("Rex");
        updatedPetDto.setBirthDate(LocalDate.of(2020, 1, 15));
        updatedPetDto.setWeight(null); // Set weight to null
    ...
    @Test
    @WithMockUser(roles = "OWNER_ADMIN")
    void testGetOwnerPetWithWeight() throws Exception {
        var owner = ownerMapper.toOwner(owners.get(0));
        given(this.clinicService.findOwnerById(1)).willReturn(owner);
        var pet = petMapper.toPet(pets.get(0));
        pet.setOwner(owner);
