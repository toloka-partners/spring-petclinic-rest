...
class OwnerRestControllerTests {
    ...
    private PetDto getTestPetWithIdAndName(final OwnerDto owner, final int id, final String name) {
        PetTypeDto petType = new PetTypeDto();
        PetDto pet = new PetDto();
        pet.id(id).name(name).birthDate(LocalDate.now()).type(petType.id(2).name("dog")).weight(new BigDecimal("13.55")).addVisitsItem(getTestVisitForPet(pet, 1));
        return pet;
    }
    ...
    void testCreatePetWithWeightSuccess() throws Exception {
        PetDto newPet = pets.get(0);
        newPet.setId(888);
        newPet.setWeight(new BigDecimal("27.55"));
        ObjectMapper mapper = new ObjectMapper();
        mapper.registerModule(new JavaTimeModule());
        mapper.setDateFormat(new SimpleDateFormat("yyyy-MM-dd"));
        mapper.disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS);
